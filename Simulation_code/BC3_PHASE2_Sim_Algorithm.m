...
% #########################################################################
% Title: (PHASE2) Benchmark III - Stages -> Process variables
...
for iTry = 1:nTry 
    ...
    for i=1:nLambda % lambdaF
        ...
        while 1
            ...
            % 1-2.Linesearch         
%             tau = tau0; 
%             gradBnll_prox = ( B - bc3_prox_B(B-tau*gradBnll,lambda,gamma,tau) )/tau;
%             Bnew = B - tau*gradBnll_prox;
%             while bc3_nll_fun_B(Bnew,b,X,Y,NN) > bc3_nll_fun_B(B,b,X,Y,NN) - tau*gradBnll(:)'*gradBnll_prox(:) + 0.5*tau*norm(gradBnll_prox,2)^2
%                 tau = tau*rho;
%                 gradBnll_prox = ( B - bc3_prox_B(B-tau*gradBnll,lambda,gamma,tau) )/tau;
%                 Bnew = B - tau*gradBnll_prox;
%             end 

            B = bc3_prox_B(B-tau*gradBnll,lambda,gamma,tau);
...
%             end 
            ...
            negloglik = bc3_nll_fun_B(B,b,X,Y,NN);               
            negloglik_penalty = negloglik+lambdas(i)*( sum( sqrt(gamma)*sqrt(sum(sum(B.^2,1),3)) ) );             
...
%             XX(Index1,:,:) = [];
    ...
    % nonzero stage mode index 
    nzeroidx = sum(sum(Bout,3),1) ~=0;
   
    % remove sparse stage mode
    Xn = {};
    for m = 1:NN
        XX = double(X{m});
        Xn{m,1} = XX(:,nzeroidx,:);
    ...
    %=========================
    % Step2: process variable
    %=========================
    
    for i=1:nLambda % lambda

        fprintf('iTry: %d \n',iTry) 
        fprintf('Step2: process variable selection \n')
        fprintf('lambda: %d (%d) \n',i,lambdas(i))
        fprintf('#sample: %d \n',NN)
...
%             % 1-2.linesearch         
%             tau = tau0; 
%             gradBnll_prox = ( Bn - bc2_prox_B(Bn-tau*gradBnll,lambda,gamma,tau) )/tau;
%             Bnew = Bn - tau*gradBnll_prox;
%             while bc2_nll_fun_B(Bnew,b,X,Y,NN) > bc2_nll_fun_B(Bn,b,X,Y,NN) - tau*gradBnll(:)'*gradBnll_prox(:) + 0.5*tau*norm(gradBnll_prox,2)^2
%                 tau = tau*rho;
%                 gradBnll_prox = ( Bn - bc2_prox_B(Bn-tau*gradBnll,lambda,gamma,tau) )/tau;
%                 Bnew = B - tau*gradBnll_prox;
%             end 

            Bn = bc2_prox_B(Bn-tau*gradBnll,lambda,gamma,tau);
...
%             % 2-2.linesearch                  
%             tau = tau0;    
%             while bc2_nll_fun_b(Bn,b-tau*gradbnll,X,Y,NN) > bc2_nll_fun_b(Bn,b,X,Y,NN) - 0.5*tau*norm(gradbnll,2)^2
%                 tau = tau*rho;
%             end 
            ...
            % objective and neg.log-likelihood
            negloglik = bc2_nll_fun_B(Bn,b,Xn,Y,NN);               
            negloglik_penalty = negloglik+lambdas(i)*( sum( sqrt(gamma)*sqrt(sum(sum(Bn.^2,3),2)) ) );             
    ...
    Idx1out=sum(sum(Boutn,3),2)==0; % process variable 
    Idx2out=sum(sum(Bout,3),1)==0; % stage
    Idx2out=Idx2out';
     
    TN=sum((Idx1org==Idx1out).*Idx1org) + sum((Idx2org==Idx2out).*Idx2org); 
    TP=sum((sum(sum(Boutn,3),2)~=0).*(sum(beta.U{1},2)~=0)) + sum((sum(sum(Bout,3),1)~=0)'.*(sum(beta.U{2},2)~=0)); 
    ...
    %% Save results 
    % save(['./Simulation_result/Benchmark/Record/Result_BC3_' ImportData '_' DataName num2str(NN) '_noise' num2str(noiselevel) '_iTry' num2str(iTry) '.mat'])
end 
elapsed_time = toc;

%% Save results 
save(['./Simulation_result/Benchmark/Result_BC3_' ImportData '_' DataName num2str(NN) '_noise' num2str(noiselevel) '.mat'])
