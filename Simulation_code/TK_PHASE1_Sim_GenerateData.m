...
% 1. Simulate coefficient tensor B with rank (2,1,2)
rng('default') 

U=2*rand(p1,r1)-1;
V=2*rand(p2,r2)-1;
W=2*rand(p3,r3)-1;
U(Index1,:)=zeros(length(Index1),r1);
V(Index2,:)=zeros(length(Index2),r2);

beta=ttensor(tenrand(rvec),arrayfun(@(k) randn(pvec(k),rvec(k)), 1:D,'UniformOutput',false));
beta.U{1}=U;
beta.U{2}=V;
beta.U{3}=W;
...
%% Save data
save(['./Simulation_data/DataTucker1_' num2str(n) '.mat'])
% save(['./Simulation_data/DataTucker2_' num2str(n) '.mat'])