...
% #########################################################################
% Title: (PHASE2) Tucker-based Method - BCPD Algorithm
% Output Path: /Simulation_result/Tucker/
% #########################################################################
...
lambdas=(0:2:30); % tuning parameters
ranks=(1:1:8); % rank
...
for iTry = 1:nTry 
    ...
    % use Higher-Order SVD for initialization
    [Tucker,U,NPC]=HOSVD_FXL(tensor(betaTilde),0.85);
    rankvec{1}=NPC; 
    
    for j=1:1 %nRank % rank
...
%             betaIN=HOSVD_FXL(tensor(betaTilde),0.99)
...
%     [M2,c] = min(tmp);
    ...
    Bout = BetaCell{I,1}; % best lambda: I(c), best rank: J(c)
    negloglik_penalty_out = negloglik_penalty_cell{I,1};
    G_record_out = G_record_cell{I,1};
    U_record_out = U_record_cell{I,1};
    V_record_out = V_record_cell{I,1};
    W_record_out = W_record_cell{I,1};
    b_record_out = b_record_cell{I,1};
    ...
    rslt=[rslt; iReport];
     ...
     %% Save results 
    % save(['./Simulation_result/Tucker/Record/Result_' ImportData '_' DataName num2str(NN) '_noise' num2str(noiselevel) '_iTry' num2str(iTry) '.mat'])
end 
elapsed_time = toc;

%% Save results 
save(['./Simulation_result/Tucker/Result_' ImportData '_' DataName num2str(NN) '_noise' num2str(noiselevel) '.mat'])
